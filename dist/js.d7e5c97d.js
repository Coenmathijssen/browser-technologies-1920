  ...
  if (number === null) {
    if (formCount <= formParts.length) {
      ...
      for (var i = 0; i < formParts.length; i++) {
        var transformPercentage = 100 * formCount; // formParts[i].style.transform = `translateX(-${transformPercentage}%)`

        formParts[i].classList = "transform".concat(transformPercentage);
      }
    }
  } else {
    ...
    for (var _i = 0; _i < formParts.length; _i++) {
      ...
      formParts[_i].classList = "transform".concat(_transformPercentage);
    }
